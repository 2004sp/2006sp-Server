...
extends BotTaskDefinition {
    ...
    public VarrockPalaceYewWoodcuttingBotTask(int value2) {
    ...
    public final void configureTaskInteractionTargets(Player player) {
        int value;
        ((Player)player).botInteractionTargetIds.clear();
        int[] integerValues = TreeDefinition.collectObjectIds(new TreeDefinition[]{TreeDefinition.YEW});
        int length = integerValues.length;
        int index = 0;
        while (index < length) {
            value = integerValues[index];
            ((Player)player).botInteractionTargetIds.add(value);
            ++index;
        }
        value = 30;
