...
public enum RedDragonhideRecipe {
    ...
    static {
        ...
        int length = redDragonhideRecipeArray.length;
        int index = 0;
        while (index < length) {
            RedDragonhideRecipe redDragonhideRecipe = redDragonhideRecipeArray[index];
            definitionsByButtonId.put(redDragonhideRecipe.buttonId, redDragonhideRecipe);
            ++index;
        }
    }

    public static RedDragonhideRecipe forButtonId(int buttonId) {
        if (definitionsByButtonId == null) {
            return null;
        }
        return (RedDragonhideRecipe)((Object)definitionsByButtonId.get(buttonId));
    }

    private RedDragonhideRecipe(int buttonId, int value22, int materialAmount, int productItemId, int quantity, int requiredLevel, double experience) {
